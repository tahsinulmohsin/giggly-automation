...
const log = createModuleLogger('gadgettrack-scraper');

/**
 * Scrape a single product page from gadgettrackbd.com
 * @param {string} url
 * @returns {Promise<object|null>}
 */
export async function scrapeGadgetTrackProduct(url) {
  try {
    ...
    // Pricing (Gadget Track BD doesn't have hidden prices, same logic as Gadget House)
    ...
    $('.woocommerce-product-gallery__image img, .product-images img, .woocommerce-product-gallery img').each((_, el) => {
      ...
      if (src && !src.includes('placeholder') && !images.includes(src)) {
        // Try to get full resolution image by removing WooCommerce thumbnail dimensions (e.g. -300x300.jpg -> .jpg)
